...
const messagesChatMockes = [
  ...
  },
  {
    id: 8,
    expediteur: "bot",
    contenu: "Je vous en prie ! N'hésitez pas si vous avez d'autres questions.",
    ...
    return conversationsMockees
      .filter(
        (conv) =>
          ...
          conv.dernierMessage.toLowerCase().includes(searchTerm.toLowerCase())
      )
      .sort((a, b) => {
        return sortBy === "date"
                ...
                <CardContent className="p-3">
                  <div className="flex items-center">
                    ...
                    <p className="text-xs text-muted-foreground ml-2 whitespace-nowrap">
                      {format(conversation.horodatage, "d MMM, HH:mm", {
                        locale: fr,
                      })}
                    </p>
                  </div>
              ...
              {messagesChatMockes.map((message) => (
                <motion.div
                  ...
                  initial={{
                    opacity: 0,
                    scale: 0.9,
                    originX: message.expediteur === "utilisateur" ? 1 : 0,
                  }}
