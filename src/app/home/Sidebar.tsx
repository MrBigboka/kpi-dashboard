...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    },
    {
      id: "files",
      icon: Folder,
      label: "Exporter des fichiers",
      path: "/files", // Mise à jour du chemin pour le fichier
    },
    {
      id: "contact",
      icon: Mail,
      label: "Support",
      path: "/contact", // Mise à jour du chemin pour le contact
    },
  ];
          ...
          {navItems.map((item) => (
            <Button
              ...
              className={cn(
                ...
                active === item.id && "bg-gray-100 dark:bg-gray-800",
                "group"
              )}
              onClick={() => {
                setActive(item.id);
            ...
            >
              <item.icon
                className={cn(
                  "transition-all",
                  isOpen ? "h-4 w-4 mr-2" : "h-5 w-5 block mx-auto"
                )}
              />
              <AnimatePresence>
                {isOpen && (
